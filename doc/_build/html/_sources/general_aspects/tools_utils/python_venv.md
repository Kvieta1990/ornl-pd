...
To deal with such complex scenarios, the `virtual environment` comes to rescue. It basically puts a certain version of Python and all the compatible dependencies in a `self-contained` environment and we can have multiple such environments that are independent from each other so they are not interfering each other. Probably, we all know the `conda environment` with this regard. We all know it because it is probably the most popular one, among all the alternatives. In fact, for many people, `conda environment` would be used interchangeably with `virtual environment`. Anyhow, it is an `environment` and who cares whether it is `donda environment`, `fonda environment`, or `whatever environment`... Yes, that is true and indeed we don't care, until we have to. So, `conda` is the name of the tool for managing the so-called `conda environment`. There are multiple ways to install the `conda` tool. `Anaconda` is one of the providers of the `conda` tool and it is probably the most notable provider, through the `Anaconda` or `Miniconda` distribution -- see [here](https://www.anaconda.com/blog/is-conda-free) for more information. There is another layer of complexity (the `channel`) which I don't want to go into here, but anyways, it is something associated with the `Anaconda` release of `conda` that we need to pay, if the organization using it has 200 or more people. ORNL decides not to pay for that and we are forced to change and to make the change, we better know something about it.

### Tools for virtual environment management

Getting back to the `virtual environment`, as I said, we have multiple alterntive tools to set up such self-contained `virtual environments`. Typical examples are [`virtualenv`](https://virtualenv.pypa.io/en/latest/), [`mamba`](https://github.com/mamba-org/mamba), [`pixi`](https://pixi.sh/latest/), [`peotry`](https://python-poetry.org/) and [`uv`](https://github.com/astral-sh/uv). Maybe there are more, but those are what I am aware of. Here I am not spelling out `conda`, because, 1) we mentioned it above and 2) it is indeed a bit confusing, since we have two different `conda`'s -- one is the `conda` from the `Anaconda` company (the one mentioned above) and the other is from the [`conda-forge`](https://conda-forge.org/) community. We said above that something associated with the `Anaconda` version of `conda` is non-free now, but the `conda-forge` community version of `conda` is all free. Each of the tools mentioned here works differently and two of them are worth special mentioning. One of them is for sure `conda` (here, I specifically mean the `conda-forge` version of `conda`) -- as being said, it is all free and we can happily use it just like `conda` (I mean the `conda` command we have been using before, which was most likely installed via `Anaconda`). Well, in terms of usage, they do not really make any differences. So, if we decide to install the `conda-forge` version of `conda`, all the `conda` commands we were happily executing before should stay the same. Another tool to specially mention is `mamba` -- it is also coming from `conda-forge` and works very much like `conda`. For many typical commands we were executing before with `conda`, like `conda env list`, `conda activate`, etc., we only need to replace `conda` with `mamba`.
